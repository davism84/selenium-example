...
public class FS_RAViewer_SC extends TestBase {

		@Test
		public void test_risk_viewer_sc() throws InterruptedException 
		{
			...
			driver.findElement(By.id("Sugar Creek")).click();
			driver.manage().timeouts().pageLoadTimeout(100, TimeUnit.SECONDS);
		...
		for (int second = 0;; second++) {
			if (second > timeoutLimit) Assert.fail("timeout");
			try { if ("0 Patient(s) with Newly Increased Risk of Falling".equals(driver.findElement(By.id("ui-id-1")).getText())) break; } catch (Exception e) {}
			Thread.sleep(1000);
		}
...
//		
//		try {
//			  assertEquals("Patient #5787", driver.findElement(By.linkText("Patient #5787")).getText());
//			} catch (Error e) {
...
//
//		try {
//			assertEquals("Situation: Non-Alzheimers Dementia. ADL Scale <= 21.",
//					driver.findElement(By.id("sit")).getText());
//		} catch (Error e) {
//			verificationErrors.append(e.toString());
//		}
//		try {
//			assertEquals("> 2 CNS Drugs (+/-) Fall Risk Agent + MDS Behavior Symptoms to other (DELERIUM)",
