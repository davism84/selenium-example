...
public class FS_RAViewer_CBP extends TestBase {

		@Test
		public void test_risk_viewer_cbp() throws InterruptedException 
		{
			...
			driver.findElement(By.id("Canterbury Place")).click();
			driver.manage().timeouts().pageLoadTimeout(100, TimeUnit.SECONDS);
		...
		for (int second = 0;; second++) {
			if (second > timeoutLimit) Assert.fail("timeout");
			try { if ("2 Patient(s) with Newly Increased Risk of Falling".equals(driver.findElement(By.id("ui-id-1")).getText())) break; } catch (Exception e) {}
			Thread.sleep(1000);
		}
...
//		add a wait for Expand button here
  ...
  //       }
		...
		for (int second = 0;; second++) {
			if (second > timeoutLimit) Assert.fail("timeout");
			try { if ("Patient #7663".equals(driver.findElement(By.linkText("Patient #7663")).getText())) break; } catch (Exception e) {}
			Thread.sleep(1000);
		}		
		
		try {
			  Assert.assertEquals("Patient #7805", driver.findElement(By.linkText("Patient #7805")).getText());
			} catch (Error e) {
